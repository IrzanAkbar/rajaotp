...
setIntervalMinutes(minutes) {
  if (minutes < 1) throw new Error("Interval minimal 1 menit");

  this.intervalMinutes = minutes;
  this.intervalMs = minutes * 60 * 1000;

  const now = Date.now();
  const nextTime = now + this.intervalMs;

  // simpan ulang ke file
  this.saveBackupInfo(now, nextTime);

  console.log(`⏱ Interval backup diubah menjadi ${minutes} menit`);
}
...
await bot.sendDocument(
  ...
  console.log("⚠️ Upload gagal, retry 1 detik...");
...
/// ⛔ FILE DIHAPUS SETELAH PASTI TERUPLOAD
setTimeout(() => {
  try {
    fs.unlinkSync(zipFullPath);
    console.log("🗑️ File ZIP dihapus setelah upload aman.");
  } catch (e) {
    console.log("⚠️ Gagal hapus ZIP:", e.message);
  }
}, 3000);
...
startAutoBackup() {
  ...
  // 🔄 WAJIB load snapshot dulu
  const lastBackup = this.getLastBackupTime();
  const now = Date.now();

  // ⛔ pakai interval dari FILE, bukan constructor
  ...
  if (!lastBackup) {
    // belum pernah backup
    firstDelay = intervalMs;
  } else if (now >= lastBackup + intervalMs) {
    // ⏱️ SUDAH TERLEWAT → kirim SEKARANG
    firstDelay = 5000; // 5 detik aman setelah restart
  } else {
    // ⏳ masih nunggu sisa waktu
    firstDelay = (lastBackup + intervalMs) - now;
  }

  this.timeoutId = setTimeout(async () => {
    await this.kirimBackupOtomatis();
  ...
  const next = moment(now + firstDelay)
    .tz("Asia/Jakarta")
    .format("DD-MM-YYYY HH:mm:ss");

  console.log("🔁 Jadwal backup berikut:", next);

  this.bot.sendMessage(
    this.adminId,
    `🔁 Auto-backup aktif kembali\n⏳ Backup berikut: ${next}`
  );
}
}
